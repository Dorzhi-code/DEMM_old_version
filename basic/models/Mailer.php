...
class Mailer extends \Yii\base\Model{
    ...
    *
    */   
    public static $smtp_username = 'pmduconf@yandex.ru';
    public static $smtp_password = 'jelgwigspucoemeb';
    public static $smtp_host = 'ssl://smtp.yandex.ru';
    public static $smtp_name = 'demm.bsu.ru';
    public static $smtp_from = 'pmduconf@yandex.ru';
    ...
    */
    private static function send($mailTo, $subject, $message) {
        ...
        try {
			...
			$server_name = 'demm.bsu.ru'; //$_SERVER["SERVER_NAME"];
...
//        $models = $params['models'];
    ...
    public static function sendPassword($email, $params) {

        
        $pass = $params['pass'];
        $lang = $params['lang'];
        if ($lang == 'ru') {
            $subject = 'Благодарим за регистрацию на сайте demm.bsu.ru!';
            $message = 'Вы регистрировались на сайте: '. date("D, d M Y H:i:s") . " UT\r\n".'<br>'.'<br>';
            $message .= 'Ваш пароль для входа в личный кабинет: '.$pass.'<br>'.'<br>';
            $message .= 'Пожалуйста, сохраните данное письмо и храните в надежном месте.'.'<br>'.'<br>';
            $message .= '-----';
            $message .= '<br>'.'<br>';
            $message .= 'Вы получили это сообщение, потому что Ваш email-адрес был зарегистрирован на сайте demm.bsu.ru'.'<br>';
            $message .= 'Если данное письмо попало к Вам по ошибке, пожалуйста, проигнорируйте его и примите наши извинения.'.'<br>'.'<br>';
            $message .= 'С наилучшими пожеланиями,'.'<br>';
            $message .= 'Организационный коммитет II Международной научной конференции "Дифференциальные уравнения и математическое моделирование"';
        }

        if ($lang == 'en') {
            $subject = 'Thank you for creating an account on demm.bsu.ru!';
            $message = 'You registered on the site: '. date("D, d M Y H:i:s") . " UT\r\n".'<br>'.'<br>';
            $message .= 'Your password to enter your account: '.$pass.'<br>'.'<br>';
            $message .= 'Please save this letter and keep it in a safe place.'.'<br>'.'<br>';
            $message .= '-----';
            $message .= '<br>'.'<br>';
            $message .= 'You received this message because your email address was registered on demm.bsu.ru.'.'<br>';
            $message .= 'If you received this email by mistake, please ignore it and accept our apologies.'.'<br>'.'<br>';
            $message .= 'Best wishes,'.'<br>';
            $message .= 'Organizing Committee of the II International Scientific Conference "Differential Equations and Mathematical Modeling".';
        }
